...
const fixturePath = fileURLToPath(new URL("./prime-agent-contract-round-trip.json", import.meta.url));
let checks = 0;
...
const instanceId = (value) => {
  ...
  if (!/^[A-Za-z][A-Za-z0-9_-]{0,63}$/.test(result)) throw new Error("invalid provider instance id");
  return result;
};
    ...
    result.options = Array.isArray(input.options)
      ...
      : Object.entries(input.options).flatMap(([id, value]) =>
          (typeof value === "string" && value.trim() !== "") || typeof value === "boolean"
            ? [{ id: trim(id, "invalid option id"), value: typeof value === "string" ? value.trim() : value }]
            : [],
        );
  }
  return result;
...
function decodePrimeSettings(input) {
  ...
  return { binaryPath: trim(input.binaryPath ?? "prime-agent", "Prime binaryPath must be non-empty") };
}
...
function bootstrap(settings) {
  ...
  return {
    ...
    providerInstances: {
      ...
      "prime-agent": { driver: "prime-agent", enabled: false, config: { binaryPath: "prime-agent" } },
    },
  };
}
function redactSettings(settings) {
  return {
    ...settings,
    providerInstances: Object.fromEntries(Object.entries(settings.providerInstances).map(([id, envelope]) => [id, {
      ...envelope,
      ...(envelope.environment ? { environment: envelope.environment.map((variable) =>
        variable.sensitive
          ? { ...variable, value: "", ...(variable.value !== "" || variable.valueRedacted ? { valueRedacted: true } : {}) }
          : Object.fromEntries(Object.entries(variable).filter(([key]) => key !== "valueRedacted")),
      ) } : {}),
    }])),
  };
}
...
try {
  ...
  same(selections.map((selection) => selection.nativeIdentity), [
  ...
  ok(selections[0].nativeIdentity.provider !== selections[1].nativeIdentity.provider,
    "same modelId must not collapse providers");
  same(selections.map((selection) => selection.model), fixture.expected.readableSlugs,
    "readable slugs are retained and never reverse-parsed");
  same(selections.map(encodeSelection).map(decodeSelection), selections,
    "selection wire round trip retains native identity and readable slug");

  const snapshot = fixture.structuredIdentity.serverSnapshot;
  const snapshotRoundTrip = JSON.parse(JSON.stringify(snapshot));
  same(snapshotRoundTrip.models.map((model) => model.slug), fixture.expected.readableSlugs,
    "server snapshot round trip retains readable slugs");
  same(snapshotRoundTrip.models.map((model) => model.nativeIdentity), selections.map((selection) => selection.nativeIdentity),
    "server snapshot round trip retains structured identity");

  const prime = decodePrimeSettings(fixture.primeInstance.config);
  same(prime, { binaryPath: "/opt/prime-agent" }, "Prime instance config has typed binary setting only");
  same(decodePrimeSettings(fixture.primeInstance.configWithOwnedOverrides),
  ...
  same(unknown, {
    driver: "fork-driver", displayName: "Fork instance",
  ...
  same(bootstrapped.providers, fixture.legacySettings.providers, "bootstrap preserves legacy provider settings");
  same(bootstrapped.textGenerationModelSelection, fixture.legacySettings.textGenerationModelSelection,
    "bootstrap preserves readable legacy selection");
  same(bootstrapped.providerInstances["prime-agent"], fixture.expected.defaultPrime,
  ...
  const oldSelection = decodeSelection({ provider: "prime-agent", model: "anthropic/claude-sonnet" });
  same(oldSelection, { instanceId: "prime-agent", model: "anthropic/claude-sonnet" },
    "old selection safely decodes when native identity is absent");
  ok(fixture.oldServerSnapshot.models[0].nativeIdentity === undefined,
  ...
  same(environment[0].value, fixture.expected.redactedSensitiveValue,
    "externally materialized sensitive environment value is redacted");
  same(environment[0].valueRedacted, fixture.expected.redactedMarker,
    "externally materialized sensitive environment carries redaction marker");
  same(environment[1].value, "https://api.prime.example", "non-sensitive environment value remains readable");
  ok(!JSON.stringify(redacted).includes("fixture-secret-token"),
    "external settings materialization never exposes sensitive environment value");

  for (const test of fixture.rejections) {
    if (test.name === "invalid instance id") reject(test.name, () => decodeSelection(test.input), test.message);
