...
const command = (type) => new Promise((resolve) => { const id=`cmd-${++sequence}`; const on=(line)=>{ const e=JSON.parse(line); events.push(e); if(e.type==='response' && e.id===id){lines.off('line',on); resolve();} }; lines.on('line',on); child.stdin.write(JSON.stringify({id,type})+'\n'); });
