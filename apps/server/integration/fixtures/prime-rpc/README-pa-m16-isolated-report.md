...
Each temporary directory is recorded in the manifest with every spawned child. The internal lifecycle record retains its exact numeric PID and Linux `/proc/<pid>/stat` start-time token for proof and signalling, but the emitted JSON projects both to `pidHash` and `startTokenHash` SHA-256 identities; no raw `pid` field or numeric PID is serialized, including in the RPC transcript. The RPC runner ends stdin after its required responses, then requires the exact child to close. A lingering owned child is reaped only after its Linux start-time token matches the launch token; missing or changed identity fails cleanup and retains the root. Cleanup must report `removed` for a ready/passed result. The `linger` fake scenario regression verifies this path. No name- or command-based process discovery is used.
