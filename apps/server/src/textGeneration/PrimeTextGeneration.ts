...
export const makePrimeTextGeneration = Effect.fn("makePrimeTextGeneration")(function* (settings: PrimeAgentSettings, options: PrimeTextGenerationOptions) {
  ...
  const run = async <T>(operation: Op, cwd: string, prompt: string, selection: ModelSelection, outputSchema: Schema.Codec<any, any, never>, convert: (x: Record<string, unknown>) => T, attachments?: ReadonlyArray<ChatAttachment>): Promise<T> => {
    ...
    const transport = (options.launch ?? spawnPrimeRpcTransport)(settings.binaryPath, ["--mode", "rpc", "--session-dir", layout.session], { cwd: cwdReal, env: safeEnv });
    ...
    const client = new PrimeRpcClient(transport, { requestIdPrefix: "t3-prime-text", defaultTimeoutMs: timeoutMs }); const controller = new AbortController(); let deadlineReject!: (error: Error) => void; const deadline = new Promise<never>((_, reject) => { deadlineReject = reject; }); void deadline.catch(() => undefined); const timer = setTimeout(() => { controller.abort(); deadlineReject(new Error("Prime text generation timed out")); if (processIdentity) void transport.stopExact?.(processIdentity).catch(() => undefined); void Promise.resolve(transport.close?.()).catch(() => undefined); }, timeoutMs); const chunks: string[] = []; let ended = false;
    try {
