...
const makeExactRemover = async (resource: PrimeOwnedResource): Promise<"removed" | "retained"> => {
  ...
  try {
    ...
    if (p.isSymbolicLink() || !p.isDirectory()) return "retained";
    parentIdentity = { dev: p.dev, ino: p.ino };
    const parentReal = await realpath(parent);
    if (!contained(parentReal, path)) return "retained";
  } catch { return "retained"; }
  let before;
  try { before = await lstat(path, { bigint: true }); } catch (e) { if ((e as NodeJS.ErrnoException).code === "ENOENT") return "removed"; return "retained"; }
  ...
  const quarantine = join(parent, `.${basename(path)}.quarantine`);
  try {
    const p = await lstat(parent, { bigint: true });
    const check = await lstat(path, { bigint: true });
    if (p.isSymbolicLink() || p.dev !== parentIdentity.dev || p.ino !== parentIdentity.ino ||
        check.isSymbolicLink() || check.dev !== before.dev || check.ino !== before.ino) return "retained";
    try { await lstat(quarantine); return "retained"; } catch (e) { if ((e as NodeJS.ErrnoException).code !== "ENOENT") return "retained"; }
    await rename(path, quarantine);
    const parentAfter = await lstat(parent, { bigint: true });
    if (parentAfter.isSymbolicLink() || parentAfter.dev !== parentIdentity.dev || parentAfter.ino !== parentIdentity.ino) return "retained";
    // Final awaited observation before deletion binds the quarantine pathname to
    // the exact inode that was renamed out of the active namespace.
  ...
  const run = async <T>(operation: Op, cwd: string, prompt: string, selection: ModelSelection, outputSchema: Schema.Codec<any, any, never>, convert: (x: Record<string, unknown>) => T, attachments?: ReadonlyArray<ChatAttachment>): Promise<T> => {
    ...
    const finalize = async () => {
      ...
      try {
        ...
        if (layout && record) {
          // A close request is not a terminal observation. Ownership remains
          // active unless the exact child has stopped and transport reported it.
          let terminalObserved = false;
          if (transport) {
            try {
              terminalObserved = await Promise.race([
                transport.terminal.then(() => true),
                new Promise<boolean>((resolve) => setTimeout(() => resolve(false), 1_500)),
              ]);
            } catch {}
          }
          if (processIdentity && terminalObserved) {
            try { record = { ...record, process: processIdentity, processStopped: true }; await writePrimeOwnership(layout.ownership, record); } catch {}
          }
          if (transport && !terminalObserved) return;
    ...
    try {
      ...
      // Register ownership before any session/tmp resource is created.
