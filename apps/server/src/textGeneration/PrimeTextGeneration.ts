...
export const makePrimeTextGeneration = Effect.fn("makePrimeTextGeneration")(function* (settings: PrimeAgentSettings, options: PrimeTextGenerationOptions) {
  ...
  const run = async <T>(operation: Op, cwd: string, prompt: string, selection: ModelSelection, outputSchema: Schema.Top, convert: (x: Record<string, unknown>) => T, attachments?: ReadonlyArray<ChatAttachment>): Promise<T> => {
    ...
    const processIdentity = await transport.processIdentityReady?.();
    if (processIdentity) await writePrimeOwnership(layout.ownership, { ...record, process: processIdentity });
    const client = new PrimeRpcClient(transport, { requestIdPrefix: "t3-prime-text", defaultTimeoutMs: TIMEOUT_MS }); const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), TIMEOUT_MS); const chunks: string[] = []; let ended = false;
    try {
      ...
      let events = 0; for await (const envelope of client.events()) { if (++events > MAX_EVENTS) throw Error("Prime emitted too many events"); if (envelope._tag !== "known-event") continue; const event = envelope.value as { type: string; message?: unknown; assistantMessageEvent?: unknown }; if (event.type === "message_update" || event.type === "message_end") appendText(event.assistantMessageEvent ?? event.message, chunks); if (event.type === "turn_end") { ended = true; break; } }
    ...
    finally { clearTimeout(timer); controller.abort(); client.close(); await Promise.resolve(transport.close?.()).catch(() => undefined); await transport.terminal.catch(() => undefined); try { await writePrimeOwnership(layout.ownership, { ...record, processStopped: true, rpcCleaned: true, resourcesCleaned: false }); } catch {} }
  };
