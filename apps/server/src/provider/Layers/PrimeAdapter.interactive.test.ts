...
describe("PrimeAdapter extension UI end-to-end",()=>{
 ...
 it.effect("cancels duplicate ids exactly once",()=>Effect.scoped(Effect.gen(function*(){const f=yield* setup,a=yield* start(f);yield* turn(a,f,"duplicate");yield* next(a);yield* next(a);const cs=yield* commands(f.marker);const rs=cs.filter((c:any)=>c.type==="extension_ui_response");assert.equal(rs.length,2);assert.deepStrictEqual(rs.map((c:any)=>c.id),["r-dup","r-dup"]);assert.equal(yield* a.hasSession(THREAD),true);})));
 it.effect("retains pending request after failed response for retry",()=>Effect.scoped(Effect.gen(function*(){const f=yield* setup; (f as any).failOnce=true; const a=yield* start(f);yield* turn(a,f,"input");yield* next(a);const first=yield* Effect.exit(a.respondToUserInput(THREAD,"r-input",{"r-input":"Alice"}));assert.equal(first._tag,"Failure");yield* a.respondToUserInput(THREAD,"r-input",{"r-input":"Alice"}); })));
 it.effect("crash clears pending session and emits terminal once",()=>Effect.scoped(Effect.gen(function*(){const f=yield* setup,a=yield* start(f);yield* turn(a,f,"crash");yield* next(a);const es=yield* Stream.runCollect(Stream.take(a.streamEvents,3));assert.equal(yield* a.hasSession(THREAD),false);assert.equal(Array.from(es).filter((e:any)=>e.type==="session.exited").length,1);})));
