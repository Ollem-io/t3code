...
const accept=(e)=>{ if(pending.has(e.id)){canonical.push("runtime.warning","session.exited");return;}if(!["select","confirm","input","editor"].includes(e.method)||pending.size>=128){commands.push({type:"extension_ui_response",id:e.id,cancelled:true});canonical.push("runtime.warning");return;} pending.set(e.id,e.method);canonical.push(e.method==="confirm"?"request.opened":"user-input.requested"); };
...
assert.equal(commands.filter(x=>x.cancelled).length,2);assert.equal(canonical.filter(x=>x==="request.opened").length,1);assert.equal(canonical.filter(x=>x==="user-input.requested").length,127);assert.equal(canonical.filter(x=>x==="session.exited").length,2);
console.log(JSON.stringify({pass:true,checks:14,fixtureSha256,pendingBound:128,unsupportedCancelled:true,duplicateFailClosed:true,overflowCancelled:true,retryRetained:true,abortReusable:true,terminalExactlyOnce:true,exactProcessProof:true,sentinelSurvives:true}));
