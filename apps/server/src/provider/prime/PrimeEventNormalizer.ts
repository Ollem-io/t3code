...
export class PrimeEventNormalizer {
  ...
  drain(envelope: PrimeRpcEnvelope): ProviderRuntimeEvent[] {
    ...
    switch(e.type) {
      ...
      case "message_update": { const m=e.message as Record<string, unknown>; const native=e.assistantMessageEvent as Record<string, unknown>; const delta=textOf(native.delta) || textOf(native.content) || textOf(m.content); const streamKind=typeof native.type === "string" && native.type.includes("reasoning") ? "reasoning" : "assistant_text"; if(delta && this.#messageItem) emit("content.delta",{streamKind,delta},{turnId:this.#turn,itemId:this.#messageItem}); const usage=m.usage as Record<string, unknown> | undefined; const used=usage && typeof usage.totalTokens === "number" ? usage.totalTokens : undefined; if(used !== undefined) emit("thread.token-usage.updated",{usage:{usedTokens:Math.max(0,Math.floor(used))}},{turnId:this.#turn}); break; }
  ...
  stop(reason: string): ProviderRuntimeEvent[] {
    ...
    out.push(this.base("runtime.error", { errorClass: "transport_error", message: reason }));
