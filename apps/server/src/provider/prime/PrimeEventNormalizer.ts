...
export class PrimeEventNormalizer {
  ...
  drain(envelope: PrimeRpcEnvelope): ProviderRuntimeEvent[] {
    ...
    switch(e.type) {
      ...
      case "extension_ui_request": {
        ...
        else if (e.method === "select" || e.method === "input" || e.method === "editor") emit("user-input.requested", { questions: [{ id: e.id, header: cleanNative(e.title), question: cleanNative(e.title), options: e.method === "select" ? e.options.slice(0, MAX_SELECT_OPTIONS).map((label) => ({ label: cleanNative(label, "Option"), description: cleanNative(label, "Option") })) : [], multiSelect: false }] }, { turnId: this.#turn, requestId, providerRefs: { providerRequestId: e.id } });
        else emit("runtime.warning", { message: "Prime Agent extension UI request was cancelled because this method is unsupported." }, { turnId: this.#turn, providerRefs: { providerRequestId: e.id } });
        break;
      }
  ...
  cancelled(id: string, reason = "Prime Agent interactive request was cancelled."): ProviderRuntimeEvent[] {
    return [this.base("runtime.warning", { message: reason.slice(0, MAX_NATIVE_STRING) }, { providerRefs: { providerRequestId: id } })];
  }

  resolved(id: string, kind: "request" | "user-input", payload: Record<string, unknown>): ProviderRuntimeEvent[] {
    const requestId = RuntimeRequestId.make(id);
    return [this.base(kind === "request" ? "request.resolved" : "user-input.resolved", payload, { requestId, providerRefs: { providerRequestId: id } })];
  }
  abort(reason = "Prime Agent turn was interrupted."): ProviderRuntimeEvent[] {
    if (this.#stopped) return [];
    const out: ProviderRuntimeEvent[] = [];
    if (this.#turn && !this.#ended) { this.#ended = true; out.push(this.base("turn.aborted", { reason }, { turnId: this.#turn })); }
    return out;
  }
