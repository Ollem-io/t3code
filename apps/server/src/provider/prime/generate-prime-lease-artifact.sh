...
artifact=apps/server/src/provider/prime/prime-lease-artifact.mjs
out="$(mktemp -d "${TMPDIR:-/tmp}/t3-prime-lease-bundle.XXXXXX")"
candidate="$out/candidate.mjs"
trap 'rm -rf "$out"' EXIT

"$vp" pack apps/server/src/provider/prime/verify-prime-lease.ts \
  --out-dir "$out" \
  --no-clean --no-sourcemap --platform node --format esm --target node24 \
  --no-report --logLevel silent
cp "$out/verify-prime-lease.mjs" "$candidate"
"$vp" fmt "$candidate"
