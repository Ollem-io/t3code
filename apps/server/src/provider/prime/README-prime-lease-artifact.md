...
SHA-256: `d01b3d88525378fbf455b36dd07be849f4da1e915586081ef0926deb4a010a6d`.
...
The verifier reports 170 source-derived assertions and a race trace covering:
...
- **liveness** — a writer whose turn outruns the TTL keeps renewing on a bounded, injected schedule, so across three TTLs a second process is refused `heldByAnotherWriter` on every attempt, the busy writer still owns its session when the turn lands, and teardown stops the schedule;
