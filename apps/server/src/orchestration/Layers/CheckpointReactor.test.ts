...
describe("CheckpointReactor", () => {
  ...
  it("completes filesystem revert with an explicit provider-history disclaimer when rollback is unsupported", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.checkpoint.revert",
        commandId: CommandId.make("cmd-unsupported-revert"),
        threadId: ThreadId.make("thread-1"),
        turnCount: 1,
        createdAt,
      }),
    ...
    expect(thread?.checkpoints).toHaveLength(1);
    expect(NodeFS.readFileSync(NodePath.join(harness.cwd, "README.md"), "utf8")).toBe("v2\n");
    const disclaimer = thread?.activities.find(
      (activity) => activity.kind === "checkpoint.revert.failed",
    );
    expect(disclaimer?.payload).toMatchObject({
      detail: expect.stringContaining("provider conversation history was not rewound"),
    });
  });
